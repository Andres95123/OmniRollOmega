...
async function cleanDist() {
  ...
  for (const browser of browsers) {
    if (fs.existsSync(browser.distDir)) {
      await execAsync(`Remove-Item -Recurse -Force ${browser.distDir}`, {
        shell: "powershell.exe",
      });
    }

    if (fs.existsSync(browser.zipName)) {
      await execAsync(`Remove-Item -Force ${browser.zipName}`, {
        shell: "powershell.exe",
      });
    }
  }
...
async function buildForBrowser(config: BrowserConfig) {
  ...
  if (fs.existsSync("dist")) {
    await execAsync(`Move-Item dist ${config.distDir}`, {
      shell: "powershell.exe",
    });
  }
